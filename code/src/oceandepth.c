 ...
 * Jeu d'exploration sous-marine - Version simplifiée et fonctionnelle
 */
...
void afficher_menu() {
    ...
    printf("3. ⚔️  Combat avec créatures\n");
    printf("4. 🏥  Statut du joueur\n");
    printf("5. 🦈  Créatures présentes\n");
    printf("6. 📋  Quête active\n");
...
void gerer_avancement_simple(CarteOceanique *carte, Plongeur *joueur) {
    ...
    printf("1⬆️ Haut  2⬇️ Bas  3⬅️ Gauche  4➡️ Droite\n");
    printf("Choix: ");
    ...
    printf("✅ Déplacement réussi vers (%d,%d)!\n", new_x, new_y);
...
void explorer_zone_simple(CarteOceanique *carte, Plongeur *joueur) {
    ...
    if (zone->exploree) {
        printf("🗺️ Zone déjà explorée: %s\n", nom_type_zone(zone->type));
        return;
    }
    
    zone->exploree = 1;
    printf("🔍 Exploration de %s terminée!\n", nom_type_zone(zone->type));
    printf("🗝️ Nouvelles zones débloquées!\n");
    
    // Débloquer zones adjacentes
    debloquer_zones_adjacentes(carte, carte->position_joueur.x, carte->position_joueur.y);
    
    // Récompense exploration
    joueur->xp += 25;
    joueur->perles += 10;
...
void combat_simple(CarteOceanique *carte, Plongeur *joueur, Inventaire *inv) {
    ...
    printf("⚔️ Un ennemi vous attaque!\n");
    ...
    while (joueur->points_de_vie > 0 && pv_ennemi > 0) {
        ...
        pv_ennemi -= degats;
        printf("🗡️ Vous attaquez: -%d PV (reste: %d)\n", degats, pv_ennemi);
    ...
    if (joueur->points_de_vie <= 0) {
        printf("💀 Défaite! Retour à la base...\n");
...
int main() {
    ...
    printf("✅ Jeu initialisé! Position: Base sous-marine\n");
    printf("🎒 Équipement de départ ajouté!\n\n");
    ...
    while (continuer) {
        ...
        if (jeu_termine(&quetes)) {
            printf("🎊 VICTOIRE TOTALE! Félicitations!\n");
            break;
        }
        ...
        switch (choix) {
            ...
            case 7:
                ...
                break;
            case 8:
                printf("💾 Sauvegarde... (fonction pas encore implémentée)\n");
                break;
            case 9:
                printf("📂 Chargement... (fonction pas encore implémentée)\n");
                break;
            case 0:
        ...
        if (zone_actuelle && zone_actuelle->profondeur > 0) {
            ...
            if (joueur.niveau_oxygene < 10) {
                printf("⚠️ Oxygène faible: %d/%d\n", joueur.niveau_oxygene, joueur.niveau_oxygene_max);
            }
            if (joueur.niveau_oxygene <= 0) {
                printf("💨 Manque d'oxygène! -10 PV\n");
