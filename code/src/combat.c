...
// Quand on gagne un combat, on donne des récompenses (perles, objets, équipements)
void donner_recompenses_victoire(struct Plongeur *plongeur, struct Inventaire *inv, int difficulte) {
    ...
    if (boss_id == 0) {
        printf("\n🏆 Vous terrassez le Léviathan ! Récompense : Harpon Électrique et 100 perles !\n");
        ajouter_objet(inv, creer_harpon_electrique());
        plongeur->perles += 100;
    } else if (boss_id == 1) {
        printf("\n🏆 Vous terrassez le Kraken ! Récompense : Armure Titanium et 120 perles !\n");
        ajouter_objet(inv, creer_combi_titanium());
        plongeur->perles += 120;
    } else if (boss_id == 2) {
        printf("\n🏆 Vous terrassez le Requin Fantôme ! Récompense : Harpon Électrique et 80 perles !\n");
        ajouter_objet(inv, creer_harpon_electrique());
        plongeur->perles += 80;
    } else {
        ...
        // 1 chance sur 4 d'obtenir un équipement
        if (rand() % 4 == 0) {
            if (rand() % 2 == 0) {
                Objet harpon = creer_harpon_rouille();
                ajouter_objet(inv, harpon);
                printf("🗡️ Vous récupérez un harpon rouillé !\n");
            } else {
                Objet combi = creer_combi_neoprene();
                ajouter_objet(inv, combi);
                printf("🛡️ Vous récupérez une combinaison néoprène !\n");
            }
        }
    }
}
...
// Ici j'applique des effets spéciaux selon le type de créature
static void apply_special_effect(Creature *c, Plongeur *p, int *player_attack_reduction, float *damage_multiplier) {
    ...
    if (!c || !p) return;
    switch (c->type) {
    case CREATURE_TYPE_BOSS: // Le boss (kraken) attaque deux fois (géré ailleurs)
            break;
    case CREATURE_TYPE_SHARK: // Le requin fait plus mal s'il a moins de 50% de PV
            if (c->hp * 2 < c->max_hp) *damage_multiplier = 1.3f;
            break;
    case CREATURE_TYPE_CRAB: // Le crabe réduit les dégâts (géré dans creature_damage)
            break;
        default:
            break;
    }
...
// Ici je fais attaquer toutes les créatures vivantes, triées par vitesse
// Ensuite j'applique les effets spéciaux si besoin
int phase_attaque_creatures(Plongeur *plongeur, CreatureList *liste) {
    if (!liste || !plongeur) return 0;
    // Je compte combien de créatures sont encore vivantes
    int count = 0;
    ...
    if (!arr) {
        fprintf(stderr, "Erreur : échec d'allocation mémoire pour le tableau de créatures.\n");
        return 0;
    }
    int i = 0;
    ...
    // Chaque créature attaque le joueur
        ...
        for (int h = 0; h < hits; ++h) {
            ...
            switch (c->type) {
                case CREATURE_TYPE_BOSS:
                    ...
                    if (hits == 2 && h == 0) printf("   (Étreinte tentaculaire - 1ère attaque)\n");
                    if (hits == 2 && h == 1) printf("   (Étreinte tentaculaire - 2ème attaque)\n");
                    break;
                case CREATURE_TYPE_SHARK:
                    printf("🦈 Le Requin fonce sur vous!\n");
                    if (dmg_mult > 1.0f) printf("   (Frénésie sanguinaire - dégâts augmentés)\n");
                    break;
                case CREATURE_TYPE_CRAB:
                    printf("🦀 Le Crabe Géant vous pince!\n");
                    break;
                default:
